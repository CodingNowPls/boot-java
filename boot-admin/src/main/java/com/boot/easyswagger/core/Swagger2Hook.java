package com.boot.easyswagger.core;

import com.boot.easyswagger.entity.ApiDoc;
import com.boot.easyswagger.entity.DocInfo;
import com.boot.easyswagger.entity.ModelDoc;
...
public class Swagger2Hook {
    ...
    private void hookSwaggerDoc(Documentation doc) {
        Map<String, List<ApiListing>> apiList = doc.getApiListings();

        for (List<ApiListing> apiListing : apiList.values()) {
            for (ApiListing listing : apiListing) {
                for (Model model : listing.getModels().values()) {
                    scanModelAndReplace(model);
                }
            }
            for (ApiListing listing : apiListing) {
                for (ApiDescription apiDescription : listing.getApis()) {
                    ...
                    if (apiDoc != null) {
                        ...
                        setTags(listing.getTags(), apiDoc.controllerDescription, apiDoc.controllerClass);
                    }
                }
            }
        }
