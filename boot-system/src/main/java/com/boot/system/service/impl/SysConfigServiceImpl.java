...
public class SysConfigServiceImpl implements ISysConfigService {
    ...
    public String selectConfigByKey(String configKey) {
        String configValue = Convert.toStr(bootCache.getCacheObject(getCacheKey(configKey)));
        ...
        if (StringUtils.isNotNull(retConfig)) {
            bootCache.setCacheObject(getCacheKey(configKey), retConfig.getConfigValue());
    ...
    public int insertConfig(SysConfig config) {
        ...
        if (row > 0) {
            bootCache.setCacheObject(getCacheKey(config.getConfigKey()), config.getConfigValue());
        }
        return row;
    }
    ...
    public void deleteConfigByIds(Long[] configIds) {
        for (Long configId : configIds) {
            ...
            bootCache.deleteObject(getCacheKey(config.getConfigKey()));
        }
    }
    ...
    public void clearConfigCache() {
        Collection<String> keys = bootCache.keys(CacheConstants.SYS_CONFIG_KEY + "*");
        bootCache.deleteObject(keys);
    }
